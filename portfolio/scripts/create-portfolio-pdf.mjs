...
<section class="projects">${projects.map((p) => `<article class="project"><a class="snapshot ${p.image === "speed-dialer" ? "phone" : ""}" href="${base}projects/${p.slug}/#demo"><img src="${media(p.image + "-poster.webp")}" alt="${p.title.replace("<br>", " ")} actual product footage"><span class="play">${play}</span><span class="duration">${p.duration}</span></a><div class="project-body"><h3>${p.title}</h3><p class="project-subtitle">${p.subtitle}</p><p class="project-description">${p.description}</p></div><a class="project-link" href="${base}projects/${p.slug}/#demo">Watch full demo ${arrow}</a></article>`).join("")}</section>
...
<div class="tools"><p class="label">TECH & TOOLS</p><p>Claude · Codex · Next.js · Supabase/Postgres · Vercel · APIs · LLMs · Voice AI · CRM integrations</p></div>
<div class="experience"><p class="label">EXPERIENCE WITH</p><div class="companies"><a href="https://incredibuild.com">Incredibuild</a><a href="https://epox.ai">Epox.ai</a><a href="https://rampedup.io">RampedUp</a><a href="https://twingo.co.il">Twingo.co.il</a><a href="https://singlestore.com">SingleStore</a></div></div>
...
try {
  ...
  const overflows = await page
    .locator(".project-description,.hero-summary,.about p,.tools p")
